...
Release notes are generated by `Scripts/release/release_notes.py` through the Cursor Python SDK (`cursor-grok-4.5-low`) against commits since the previous tag. The orchestrator writes German Markdown (summary plus Features / Bug Fixes / Weitere Verbesserungen) and appends an untranslated PR or commit reference list. Token usage is printed to the Actions log; billing/metering can be added later.
